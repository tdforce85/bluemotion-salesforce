...
export default class BmcOverEngineeredLeadForm extends LightningElement {
  ...
  enterpriseSteps = [
    {
      ...
      commentary:
        "Initializing first-party data capture sequence with custom LWC inputs, real-time duplicate detection, and @track decorator magic."
    },
    {
      number: 2,
      ...
      commentary:
        "Geocoding your address via API callout. Rendering interactive map. Preparing Agentforce Drone Army deployment protocols."
    },
    {
      number: 3,
      title: "Company Intelligence",
      subtitle:
        "Normally this is where we'd charge $50k for a Data Cloud integration",
      commentary:
        "Enriching company data with excessive API calls. Today it's free because this is a demo."
    },
    {
      number: 4,
      ...
      commentary:
        "Evaluating submission against enterprise governance framework. Spoiler: It was always going to be approved."
    },
    {
      number: 5,
      ...
      commentary:
        "Firing Platform Event → Consuming in Trigger → Invoking Queueable Apex. Total fields saved so far: 2."
    },
    {
      number: 6,
      ...
      commentary:
        "Collecting supplementary metadata for the marketing automation we haven't built yet."
    },
    {
      number: 7,
      title: "Preferences",
      ...
      commentary:
        "Configuring communication preferences. Don't worry, we'll ignore these later."
    },
    {
      number: 8,
      ...
      commentary:
        "Displaying enterprise-grade terms of service. Section 1: You agree this is over-engineered. Section 2: See Section 1."
    },
    {
      number: 9,
      ...
      commentary:
        "Re-validating all inputs with redundant validation rules, duplicate rules, and workflow rules that could've been triggers."
    },
    {
      number: 10,
      ...
      commentary:
        "Simulating approval process. Status: Pending... Pending... Approved! (It was instant, we just made you wait.)"
    },
    {
      number: 11,
      title: "The Reveal",
      ...
      commentary:
        "If this made you laugh and slightly concerned about my priorities, imagine what I could build for your org."
    }
  ];
  ...
  normalSteps = [
    {
      ...
      commentary:
        "Just name, email, and company. Like a reasonable person would design."
    }
  ];
  ...
  disconnectedCallback() {
    // Tear down every timer so nothing fires on a destroyed instance.
    ...
    this._eventBusTimers.forEach((t) => clearTimeout(t));
    this._droneTimers.forEach((t) => clearTimeout(t));
  }
  ...
  get nextButtonLabel() {
    ...
    if (this.currentStep === 10) {
      return this.archReviewApproved
        ? "Approved! Proceeding..."
        : "Awaiting Approval...";
    }
  ...
  get modeToggleLabel() {
    return this.enterpriseMode
      ...
      : "Switch to Enterprise Architecture Mode";
  }
  ...
  startFakeProgressSimulator() {
    if (!this.enterpriseMode) return;
    if (this._progressInterval) return; // already running
      ...
      if (this.currentStep === 4 && this.fakeProgress < 90) {
        ...
        if (this.fakeProgress >= 90 && !this.progressGoingBackwards) {
          ...
          this._progressTimer = setTimeout(() => {
            ...
            this.showToast(
              "Progress Update",
              "Sorry, we found more architecture to add.",
              "warning"
            );
          }, 500);
        }
  ...
  incrementPlatformFeatures(feature) {
    ...
    console.log(
      `🎯 Platform Feature Invoked [${this.platformFeaturesInvoked}]: ${feature}`
    );
  ...
  handleFieldChange(event) {
    ...
    if (value && value.trim()) {
      this.fieldsCaptured = Object.values(this.formData).filter(
        (v) => v && v.trim()
      ).length;
  ...
  async checkDuplicates() {
    ...
    try {
      ...
      console.log(
        "🏁 Duplicate check complete. showDuplicateModal =",
  ...
  handleDroneDeployment() {
    ...
    this.incrementPlatformFeatures(
      "Agentforce Drone Army deployment initiated"
    );

    // Clear any in-flight chain in case the user re-clicks.
    this._droneTimers.forEach((t) => clearTimeout(t));
    this._droneTimers = [];

    this._droneTimers.push(
      // eslint-disable-next-line @lwc/lwc/no-async-operation
      setTimeout(() => {
  ...
  closeTermsModal() {
    ...
    this.currentStep++;
    this.incrementPlatformFeatures(
      "Terms & Conditions acceptance (without reading)"
    );
  }
  ...
  simulateEventBus() {
    ...
    // Clear any prior chain in case the user bounced back into step 5.
    this._eventBusTimers.forEach((t) => clearTimeout(t));
    this._eventBusTimers = [];

    this._eventBusTimers.push(
      // eslint-disable-next-line @lwc/lwc/no-async-operation
      setTimeout(() => {
        this.addToDebugLog(
          "[TRIGGER] LeadSubmissionEventTrigger.trigger executed"
        );
    ...
    this._eventBusTimers.push(
      ...
      setTimeout(() => {
        this.addToDebugLog(
          "[QUEUEABLE] LeadEnrichmentQueueable.execute() invoked"
        );
  ...
  simulateArchReview() {
    ...
    this.addToDebugLog(
      "[APPROVAL] Submitted to Architectural Committee for review"
    );

    this._archReviewTimers.push(
      // eslint-disable-next-line @lwc/lwc/no-async-operation
      setTimeout(() => {
        this.archReviewStage = 1;
        this.addToDebugLog(
          "[APPROVAL] Still pending. Committee is debating tab vs. spaces."
        );
      }, 1500)
    );
    ...
    this._archReviewTimers.push(
      ...
      setTimeout(() => {
        ...
        this.addToDebugLog(
          "[APPROVAL] APPROVED. (It was always going to be approved.)"
        );
    ...
    this._archReviewTimers.push(
      ...
      setTimeout(() => {
        ...
        if (this.currentStep === 10 && this.enterpriseMode) {
          this.currentStep = 11;
          this.incrementPlatformFeatures(
            "Auto-advance after architectural review"
          );
        }
      }, 4200)
    );
  ...
  validateCurrentStep() {
    ...
    if (this.currentStep === 1) {
      if (!this.formData.lastName) {
        errorMessage =
          "Last Name is required (obviously, we need to address you formally)";
        isValid = false;
      } else if (!this.formData.email) {
        errorMessage =
          "Email is required (how else will we spam... I mean, nurture you?)";
        isValid = false;
      } else if (!this.formData.email.includes("@")) {
        errorMessage =
          "Email must contain @ symbol (basic email validation engaged)";
        isValid = false;
      }
  ...
  async handleSubmit() {
    ...
    try {
      ...
      this.showToast(
        ...
        "Despite invoking " +
          this.platformFeaturesInvoked +
          " platform features, we still failed: " +
          error.body?.message,
        "error"
      );
    } finally {
